...
def plot_berridge_bar(vals, stds, labels, sname="", clear_background=True, large_font=True, empty_plot=True, color="blue"):
    ...
    width = 0.7
    plt.bar(xs, vals, width, label=labels, alpha=0.8,
...
def berridge_plot(learning_rate, beta, steps_per_reversal, N_agents=10, empty_plot=True, large_font=True, clear_background=True, group_by_type=False):
    ...
    for i in range(N_agents):
        agent = Reward_Basis_Learner(
            gamma, [salt_r1, salt_r2], env, learning_rate, beta, [1, 0], random_policy=True)
    ...
    Vss_td = []
    for i in range(N_agents):
        TD_agent = TD_Learner(gamma, salt_total_reward,
                              env, learning_rate, beta, random_policy=True)
        rs1, V1s, a1s = TD_agent.interact(
            steps_per_reversal, return_actions=True)
        V = TD_agent.V
        Vss_td.append(V)
    ...
    alpha_normal_salt = 1
    V_normal_juice = V_juice[0] * alpha_normal_juice + \
        V_juice[1] * alpha_normal_salt
    V_normal_salt = V_salt[0] * alpha_normal_juice + \
        V_salt[1] * alpha_normal_salt

    alpha_depleted_salt = 3.3
    alpha_depleted_juice = 4.5
    V_depleted_juice = V_juice[0] * alpha_depleted_juice + \
        V_juice[1] * alpha_depleted_salt
    V_depleted_salt = V_salt[0] * alpha_depleted_juice + \
        V_salt[1] * alpha_depleted_salt
    ...
    if group_by_type:
        plot_berridge_bar([TD_V_juice, TD_V_juice, TD_V_salt, TD_V_salt], [TD_STD_juice, TD_STD_juice, TD_STD_salt, TD_STD_salt], labels=["Juice homeostasis", "Juice depleted",
                          "Salt Homeostasis", "Salt Depleted"], sname="TD_bar", clear_background=clear_background, large_font=large_font, empty_plot=empty_plot, color="blue")
        plot_berridge_bar([V_normal_juice, V_depleted_juice, V_normal_salt, V_depleted_salt], [RB_std_juice, RB_std_juice, RB_std_salt, RB_std_salt], labels=[
                          "Juice homeostasis", "Juice depleted", "Salt Homeostasis", "Salt Depleted"], sname="RB_bar", clear_background=clear_background, large_font=large_font, empty_plot=empty_plot, color="orange")
        plot_berridge_bar([normal_juice, depleted_juice, normal_salt, depleted_salt], [normal_juice_std, depleted_juice_std, normal_salt_std, depleted_salt_std], labels=[
                          "Juice homeostasis", "Juice depleted", "Salt Homeostasis", "Salt Depleted"], sname="experiment_bar", clear_background=clear_background, large_font=large_font, empty_plot=empty_plot, color="green")
        ...
        fig, ax = plt.subplots(figsize=(12, 10))
        width = 0.2
        bar_td = ax.bar(xs + (width + 0.02), [TD_V_juice, TD_V_juice], width,
                        label="Temporal Difference", alpha=0.8, yerr=[TD_STD_juice, TD_STD_juice], capsize=4)
        bar_rb = ax.bar(xs, [V_normal_juice, V_depleted_juice], width, label="Reward Basis", alpha=0.8, yerr=[
                        RB_std_juice, RB_std_juice], capsize=4)
        bar_exp = ax.bar(xs - (width + 0.02), [normal_juice, depleted_juice], width,
                         label="Experiment", alpha=0.8, yerr=[normal_juice_std, depleted_juice_std], capsize=4)
        if large_font:
            ax.set_ylabel("Approach/Nibbles/Sniffs", fontsize=40)
        else:
            ax.set_ylabel("Approach/Nibbles/Sniffs", fontsize=28)
        ...
        if empty_plot:
            plt.savefig(
                "figures/berridge_juice_barchart_empty.png", format="png")
        else:
        ...
        fig, ax = plt.subplots(figsize=(12, 10))
        width = 0.2
        bar_td = ax.bar(xs + (width + 0.02), [TD_V_salt, TD_V_salt], width,
                        label="Temporal Difference", alpha=0.8, yerr=[TD_STD_salt, TD_STD_salt], capsize=4)
        bar_rb = ax.bar(xs, [V_normal_salt, V_depleted_salt], width, label="Reward Basis", alpha=0.8, yerr=[
                        RB_std_salt, RB_std_salt], capsize=4)
        bar_exp = ax.bar(xs - (width + 0.02), [normal_salt, depleted_salt], width,
                         label="Experiment", alpha=0.8, yerr=[normal_salt_std, depleted_salt_std], capsize=4)
        if large_font:
            ax.set_ylabel("Approach/Nibbles/Sniffs", fontsize=40)
        else:
            ax.set_ylabel("Approach/Nibbles/Sniffs", fontsize=28)
        ...
        if empty_plot:
            plt.savefig(
                "figures/berridge_salt_barchart_empty.png", format="png")
        else:
...
def plot_reward_figure(rs, rs_stds=None, sname=None, title="Reward During Reversal for Temporal Difference learner"):
    ...
    plt.axvline(len(xs)//2, color="green", linestyle="--",
                linewidth="1.5", label="Reversal Time")
...
def plot_actions_figure(actions, actions_stds=None, sname=None, title="Choice selected before and after reversal"):
    ...
    if actions_stds is not None:
        plt.fill_between(xs, actions - actions_stds,
                         actions + actions_stds, alpha=0.5)
    sns.despine(left=False, top=True, right=True, bottom=False)
    plt.xlabel("Timestep", fontsize=20)
    plt.ylabel("Choice at each time-step", fontsize=20)
    plt.title(title, fontsize=20)
    plt.axvline(len(actions)//2, color="green", linestyle="--",
                linewidth="1.5", label="Reversal Time")
...
def plot_combined_figure(td_means, td_stds, rb_means, rb_stds, sname=None, title="Choice selected before and after reversal", ylabel_label="Mean Choice", label_yaxis=False, empty_plot=False, smooth=False, use_standard_error=False):
    ...
    plt.axvline(len(td_means)//2, color="green", linestyle="--",
                linewidth="1.5", label="Reversal")
    sns.despine(left=False, top=True, right=True, bottom=False)
    if not empty_plot:
...
def run_sea_salt_experiment(learning_rate, beta, gamma, steps_per_reversal, plot_results=False, salt_extinction=False):
    ...
    RB_agent = Reward_Basis_Learner(
        gamma, [salt_r1, salt_r2], env, learning_rate, beta, [1, 0])
    rs_rb, vs_rb, as_rb = sea_salt_reversal_protocol(
        RB_agent, steps_per_reversal, salt_r1, salt_r2, RB_learner=True, salt_extinction=salt_extinction)
    rs_td, vs_td, as_td = sea_salt_reversal_protocol(
        TD_agent, steps_per_reversal, salt_total_reward, salt_total_reward_after, salt_extinction=salt_extinction)
    homeostatic_agent = Homeostatic_TD_Learner(
        gamma, salt_total_reward, env, learning_rate, beta, kappa=None, simulated_reward_update=True)
    rs_k, vs_k, as_k = sea_salt_reversal_protocol(
        homeostatic_agent, steps_per_reversal, salt_total_reward, salt_total_reward_after, salt_extinction=salt_extinction)
    if plot_results:
        plot_reward_figure(
            rs_rb, title="Reward During Reversal for Temporal Difference Learner")
        plot_reward_figure(
            rs_td, title="Reward During Reversal for Reward Basis Learner")
        plot_actions_figure(
            as_rb, title="Choices of Reward Basis Learner during Reversal")
        plot_actions_figure(
            as_td, title="Choices of Temporal Difference Learner during Reversal")
...
def run_N_sea_salt_experiment(N_runs, learning_rate, beta, gamma, steps_per_reversal, salt_extinction=False, plot_combined_figure_flag=True, empty_plot=False, use_homeostatic=False):
    ...
    for i in range(N_runs):
        rs_rb, as_rb, rs_td, as_td, rs_k, as_k = run_sea_salt_experiment(
            learning_rate, beta, gamma, steps_per_reversal, salt_extinction=salt_extinction)
        rs_rbs.append(rs_rb)
        as_rbs.append(as_rb)
        rs_tds.append(rs_td)
        as_tds.append(as_td)
    ...
    if salt_extinction:
        plot_reward_figure(rs_rbs_mean, rs_rbs_std, title="Reward During Reversal for Reward Basis Learner",
                           sname="figures/sea_salt_extinction_rb_reward.png")
        plot_reward_figure(rs_tds_mean, rs_tds_std, title="Reward During Reversal for Termporal Difference Learner",
                           sname="figures/sea_salt_extinction_td_reward.png")
        plot_actions_figure(as_rbs_mean, as_rbs_std, title="Choices of Reward Basis Learner during Reversal",
                            sname="figures/sea_salt_extinction_rb_choices.png")
        plot_actions_figure(as_tds_mean, as_tds_std, title="Choices of Temporal Difference Learner during Reversal",
                            sname="figures/sea_salt_extinction_td_choices.png")
        plot_reward_figure(rs_ks_mean, rs_ks_std, title="Reward During Reversal for Homeostatic Temporal Difference Learner",
                           sname="figures/sea_salt_extinction_homeostatic_reward.png")
        plot_actions_figure(as_ks_mean, as_ks_std, title="Choices of Homeostatic Temporal Difference Learner During Reversal",
                            sname="figures/sea_salt_extinction_homeostatic_choices.png")

    if plot_combined_figure_flag:
        plot_combined_figure(rs_tds_mean, rs_tds_std, rs_rbs_mean, rs_rbs_std, title="Reward for RB/TD Learner",
                             sname="figures/empty_extinction_proper_sea_salt_rbtd_reward_empty.png", ylabel_label="Mean Reward", empty_plot=empty_plot)
        plot_combined_figure(as_tds_mean, as_tds_std, as_rbs_mean, as_rbs_std, title="Choices of RB/TD Learner",
                             sname="figures/empty_extinction_proper_sea_salt_rbtd_choices_empty.png", label_yaxis=True, empty_plot=empty_plot, smooth=True)
        # triple for homeostatic
        if use_homeostatic:
            plot_triple_combined_figure(rs_tds_mean, rs_tds_std, rs_rbs_mean, rs_rbs_std, rs_ks_mean, rs_ks_std, title="Reward for RB/TD Learner",
                                        sname="figures/no_homeostatic_proper_triple_sea_salt_rbtd_reward_2.png", ylabel_label="Mean Reward")
            plot_triple_combined_figure(as_tds_mean, as_tds_std, as_rbs_mean, as_rbs_std, as_ks_mean, as_ks_std, title="Choices of RB/TD Learner",
                                        sname="figures/no_homeostatic_proper_triple_sea_salt_rbtd_choices_2.png", label_yaxis=True)
    else:
        plot_reward_figure(rs_rbs_mean, rs_rbs_std, title="Reward During Reversal for Reward Basis Learner",
                           sname="figures/sea_salt_rb_reward.png")
        plot_reward_figure(rs_tds_mean, rs_tds_std, title="Reward During Reversal for Termporal Difference Learner",
                           sname="figures/sea_salt_td_reward.png")
        plot_actions_figure(as_rbs_mean, as_rbs_std, title="Choices of Reward Basis Learner during Reversal",
                            sname="figures/sea_salt_rb_choices.png")
        plot_actions_figure(as_tds_mean, as_tds_std, title="Choices of Temporal Difference Learner during Reversal",
                            sname="figures/sea_salt_td_choices.png")
        plot_reward_figure(rs_ks_mean, rs_ks_std, title="Reward During Reversal for Homeostatic Temporal Difference Learner",
                           sname="figures/sea_salt_homeostatic_reward.png")
        plot_actions_figure(as_ks_mean, as_ks_std, title="Choices of Homeostatic Temporal Difference Learner During Reversal",
                            sname="figures/sea_salt_homeostatic_choices.png")
...
if __name__ == '__main__':
    ...
    berridge_plot(learning_rate, beta, steps_per_reversal,
                  large_font=False, group_by_type=GROUP_BY_TYPE)
    run_N_sea_salt_experiment(20, learning_rate, beta_random_exploration, gamma, steps_per_reversal,
                              salt_extinction=salt_extinction, empty_plot=True, use_homeostatic=True)
