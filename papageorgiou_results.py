...
def plot_rb_1D():
    ...
    da_food_switch = baseline + (alpha_food_valued * delta_switch) + (alpha_sucrose_devalued * -1 * delta_switch)
    da_food_expected = baseline + (alpha_food_valued * delta_expected) + (alpha_sucrose_devalued * -1 * delta_expected)
    da_sucrose_switch = baseline + (alpha_food_valued * -1 * delta_switch) + (alpha_sucrose_devalued * delta_switch)
    da_sucrose_expected = baseline + (alpha_food_valued * -1 * delta_expected) + (alpha_sucrose_devalued * delta_expected)
    ...
    plt.axhline(baseline, color="grey", linestyle="--",linewidth="1.5",label="Baseline Dopamine")
...
def plot_pp_2AB():
    ...
    ax.set_ylabel('Dopamine Level',fontsize=25)
    ax.set_title('Dopamine Levels for Valued/Devalued at Cue Onset',fontsize=25)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.legend()
    ax.tick_params(axis='x', which='major', labelsize=20)
...
def plot_rb_2AB():
    ...
    food_received_valued = baseline + (alpha_food_valued * delta_food_food_received) + (alpha_sucrose_devalued * delta_sucrose_food_received)
    food_received_devalued = baseline + (alpha_food_devalued * delta_food_food_received) + (alpha_sucrose_valued * delta_sucrose_food_received)
    sucrose_received_valued = baseline +  (alpha_food_devalued * delta_food_sucorse_received) + (alpha_sucrose_valued * delta_sucrose_sucrose_received)
    sucrose_received_devalued = baseline + (alpha_food_valued * delta_food_sucorse_received) + (alpha_sucrose_devalued *  delta_sucrose_sucrose_received)
...
def pp_2A(V_foods, V_waters):
    ...
    total_ds_water  = []
    for i in range(len(V_foods)):
        total_ds_food.append((a_food * deltas_food_received[i,0]) + (a_water * deltas_food_received[i,1]))
        total_ds_water.append((a_food * deltas_water_received[i,0]) + (a_water * deltas_water_received[i,1]))
    print(total_ds_food)
    ...
    width = 0.2
    fig, ax = plt.subplots(figsize=(12,10))
    bar1 = ax.bar(x - width/1.5, PP_means, width, label='Experiment',alpha=0.5)
    bar2 = ax.bar(x + width/1.5, sim_means, width,yerr=[std_ds_food,std_ds_water], label='Simulation')
...
def PP_2C(V_foods, V_waters, average_conditions = False, return_data = False, input_baselining = True):
    ...
    for i in range(len(V_foods)):
        V_food = V_foods[i,:]
        V_water = V_waters[i,:]
        deltas_food_more_food.append((s1_p1 * (r_more - V_food[0])) + (s2_p1 * (r_more - V_food[1])))
        deltas_water_more_food.append((s1_p2 * (0 -V_water[0])) + (s2_p2 * (0 - V_water[1])))
        deltas_food_more_water.append((s1_p1 * (0 - V_food[0])) + (s2_p1 * (0 - V_food[1])))
        deltas_water_more_water.append((s1_p2 *  (r_more - V_water[0])) + (s2_p2 * (r_more - V_water[1])))
    ...
    for i in range(len(V_foods)):
        ds_more_food_valued.append((a_food_valued * deltas_food_more_food[i]) + (a_water_devalued * deltas_water_more_food[i]))
        ds_more_food_devalued.append((a_food_devalued * deltas_food_more_food[i]) + (a_water_valued * deltas_water_more_food[i]))
        ds_more_water_valued.append((a_food_devalued * deltas_food_more_water[i]) + (a_water_valued * deltas_water_more_water[i]))
        ds_more_water_devalued.append((a_food_valued * deltas_food_more_water[i]) + (a_water_devalued * deltas_water_more_water[i]))
    ...
    if average_conditions:
        ...
        sim_means = [(mean_food_valued + mean_water_valued)/2, (mean_food_devalued + mean_water_devalued) / 2]
        sim_stds = [(std_food_valued + std_water_valued), (std_food_devalued + std_water_devalued)]

    else:
        labels = ["MORE Food Valued", "MORE Food Devalued", "MORE Sucrose Valued", "MORE Sucrose Devalued"]
        PP_means = [PP_food_valued, PP_food_devalued,PP_sucrose_valued,PP_sucrose_devalued]
        sim_means = [mean_food_valued, mean_food_devalued, mean_water_valued, mean_water_devalued]
        sim_stds = [std_food_valued,std_food_devalued, std_water_valued, std_water_devalued]
    ...
    fig, ax = plt.subplots(figsize=(12,10))
    bar1 = ax.bar(x - width/1.5, PP_means, width, label='Experiment',alpha=0.5)
    bar2 = ax.bar(x + width/1.5, sim_means, width,yerr=sim_stds, label='Simulation')
...
def PP_2D(V_foods, V_waters, return_data = False, input_baselineing = True):
    ...
    for i in range(len(V_foods)):
        ...
        deltas_food_switch.append((a_food_valued * deltas_food_food_switch[i]) + (a_water_devalued * deltas_water_food_switch[i]))
        deltas_water_switch.append((a_food_valued * deltas_food_water_switch[i]) + (a_water_devalued * deltas_water_water_switch[i]))
        deltas_food_expected.append((a_food_valued * deltas_food_food_expected[i]) +(a_water_devalued * deltas_water_food_expected[i]))
        deltas_water_expected.append((a_food_valued * deltas_food_water_expected[i]) + (a_water_devalued * deltas_water_water_expected[i]))
    ...
    labels = ["Valued SWITCH", "Devalued SWITCH", "Valued Expected", "Devalued Expected"]
        
    PP_means = [PP_food_switch, PP_sucrose_switch,PP_food_expected,PP_sucrose_expected]
    sim_means = [mean_delta_food_switch, mean_delta_water_switch, mean_delta_food_expected, mean_delta_water_expected]
    sim_stds = [std_delta_food_switch,std_delta_water_switch, std_delta_food_expected, std_delta_water_expected]
...
def cosyne_combined_megaplot(V_foods, V_waters, empty_plot = False):
    MORE_PP_means, MORE_sim_means, MORE_sim_stds = PP_2C(V_foods, V_waters, return_data = True, average_conditions = True)
    SWITCH_PP_means, SWITCH_sim_means, SWITCH_sim_stds = PP_2D(V_foods, V_waters, return_data = True)
    labels = ["Valued Expected","Devalued Expected", "MORE Valued", "MORE Devalued", "SWITCH Valued", "SWITCH Devalued"]
    PP_means = [SWITCH_PP_means[2], SWITCH_PP_means[3],MORE_PP_means[0], MORE_PP_means[1],SWITCH_PP_means[0], SWITCH_PP_means[1]]
    sim_means = [SWITCH_sim_means[2], SWITCH_sim_means[3],MORE_sim_means[0], MORE_sim_means[1],SWITCH_sim_means[0], SWITCH_sim_means[1]]
    sim_stds = [SWITCH_sim_stds[2], SWITCH_sim_stds[3],MORE_sim_stds[0], MORE_sim_stds[1],SWITCH_sim_stds[0], SWITCH_sim_stds[1]]
...
def PP_4A(V_foods, V_waters, empty_plot = False, use_baseline = False):
    ...
    for i in range(len(V_foods)):
        total_ds_food.append((a_food * deltas_food_received[i,0]) + (a_water * deltas_food_received[i,1]))
        total_ds_water.append((a_food * deltas_water_received[i,0]) + (a_water * deltas_water_received[i,1]))
        total_ds_baseline.append((deltas_food_received[i,0] + deltas_food_received[i,1] + deltas_water_received[i,0] + deltas_water_received[i,1])/4)
    ...
    bar1 = ax.bar(x - width/1.5, PP_means, width, label='Experiment',alpha=0.5)
    if use_baseline:
        bar2 = ax.bar(x + width/1.5, sim_means, width,yerr=[std_ds_baseline,std_ds_food,std_ds_water], label='Simulation')
    else:
        bar2 = ax.bar(x + width/1.5, sim_means, width,yerr=[std_ds_food,std_ds_water], label='Simulation')
    if not empty_plot:
    ...
    if use_baseline:
        ...
        plt.savefig("figures/proper_PP_4A_4_empty_no_baseline.jpg", format="jpg")
    plt.show()
