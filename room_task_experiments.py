...
def room_r1(env, position,print_det=False,simulated=False, neg_reward = -1, pos_reward = 5):
  xpos,ypos = position
  if print_det:
    print("in r1: " + str(position) + "  " + str(env.state[xpos, ypos]) + " " + str(env.p1_rewarded))
...
def room_r2(env, position, print_det = False,simulated=False, neg_reward = -1, pos_reward = 5):
  xpos,ypos = position
  if print_det:
...
def room_r3(env, position,print_det=False,simulated=False, neg_reward = -1, pos_reward = 5):
  if print_det:
...
def room_all_reversal_protocol(agent, steps_per_reversal, r1, r_all, RB_learner = False):
  ...
  if RB_learner:
    #print("RB learner")
    #plt.imshow(agent.V.reshape(6,6))
    #plt.show()
    agent.alphas = [0,1,1]
...
def plot_reward_figure(rs,rs_stds = None,sname=None,title="Reward During Reversal for Temporal Difference learner", N_reversals = 3):
  ...
  plt.ylabel("Total reward per episode",fontsize=20)
...
def plot_results(learning_rate, beta, gamma,steps_per_reversal, plot_results = False, use_successor_agent = False, plot_successor_matrix = False,preset_env=None,use_homeostatic_agent = False):
    ...
    RB_agent = Reward_Basis_Learner(gamma,[room_r1, room_r2, room_r3],deepcopy(env),learning_rate,beta,[1,0,0])
    if use_successor_agent:
      print("INITING successor agent")
      SR_agent = SuccessorRepresentationLearner(gamma, room_r2, deepcopy(env), learning_rate, beta)
    if use_homeostatic_agent:
      H_agent = Homeostatic_TD_Learner(gamma, room_r2, deepcopy(env), learning_rate, beta)
    
    rs_rb,vs_rb = room_triple_reversal_protocol(RB_agent,steps_per_reversal,room_r1, room_r2, room_r3,RB_learner = True)
    rs_td,vs_td = room_triple_reversal_protocol(TD_agent,steps_per_reversal,room_r1, room_r2, room_r3, RB_learner = False)
    if use_successor_agent:
      rs_sr, vs_sr = room_triple_reversal_protocol(SR_agent, steps_per_reversal,room_r1, room_r2, room_r3,RB_learner = False)
      if plot_successor_matrix:
        ...
        plt.show()
    if use_homeostatic_agent:
      rs_h, vs_h = room_triple_reversal_protocol(H_agent, steps_per_reversal, room_r1, room_r2, room_r3, RB_learner=False, homeostatic_learner=True)
    if plot_results:
      plot_reward_figure(rs_rb,title="Reward During Reversal for Reward Basis Learner")
      plot_reward_figure(rs_td,title="Reward During Reversal for Temporal Difference Learner")
      if use_successor_agent:
        plot_reward_figure(rs_sr, title="Reward During Reversal for Successor Agent")
...
def plot_combined_experiment(learning_rate, beta, gamma, steps_per_reversal, plot_results = True,use_successor_agent = False, plot_successor_matrix = False, preset_env=None):
  ...
  RB_agent = Reward_Basis_Learner(gamma,[room_r1, room_r2,room_r3],env,learning_rate,beta,[1,0,0])
  if use_successor_agent:
    print("INITING successor agent")
    SR_agent = SuccessorRepresentationLearner(gamma, room_r1, env, learning_rate, beta)
  
  rs_rb,vs_rb = room_all_reversal_protocol(RB_agent,steps_per_reversal,room_r1, room_combined,RB_learner = True)
  rs_td,vs_td = room_all_reversal_protocol(TD_agent,steps_per_reversal,room_r1, room_combined,RB_learner = False)
  if use_successor_agent:
    rs_sr, vs_sr = room_all_reversal_protocol(SR_agent, steps_per_reversal,room_r1, room_combined,RB_learner = False)
    if plot_successor_matrix:
      M = SR_agent.M
      V = SR_agent.V
      print(V.shape)
      ...
      plt.show()
  if plot_results:
    plot_reward_figure(rs_rb,title="Reward During Reversal for Reward Basis Learner",N_reversals = 1)
    plot_reward_figure(rs_td,title="Reward During Reversal for Temporal Difference Learner",N_reversals = 1)
    if use_successor_agent:
      plot_reward_figure(rs_sr, title="Reward During Reversal for Successor Agent",N_reversals = 1)
...
def plot_N_results(N_results, learning_rate, beta, gamma, steps_per_reversal,results_fn = plot_results, combined_figure_flag = True, use_successor_agent = False, plot_results = True, preset_env=None, use_homeostatic_agent= False,save_data=False,data_sname="", use_standard_error = True, run_afresh = True):
  ...
  if plot_results:
    if combined_figure_flag:
      if use_successor_agent and use_homeostatic_agent:
        plot_quadruple_combined_figure(rs_tds_mean, rs_tds_std, rs_rbs_mean, rs_rbs_std, rs_srs_mean, rs_srs_std,rs_h_mean, rs_h_std, steps_per_reversal, title="Reward for RB, TD, SR, and Homeostatic", sname="figures/room_task_quadruple_"+str(learning_rate) + ".png", N_reversals=3)
      elif use_successor_agent:
        plot_triple_combined_figure(rs_tds_mean, rs_tds_std, rs_rbs_mean, rs_rbs_std, rs_srs_mean, rs_srs_std, steps_per_reversal, title="Reward during reversal for RB, TD, and SR", sname="figures/combined_room_task_reward_3_" + str(learning_rate) + ".png",N_reversals=3)
      elif use_homeostatic_agent:
        plot_triple_combined_figure(rs_tds_mean, rs_tds_std, rs_rbs_mean, rs_rbs_std, rs_srs_mean, rs_srs_std, steps_per_reversal, title="Reward during reversal for RB, TD, and Homeostatic", sname="figures/homeostatic_combined_room_task_reward_3_" + str(learning_rate) + ".png",N_reversals=3, final_label = "Homeostatic")
      else:
        plot_combined_figure(rs_tds_mean, rs_tds_std, rs_rbs_mean, rs_rbs_std, title="Reward During Reversal for TD and RB Learner", sname="figures/combined_room_task_reward_2_" + str(learning_rate) + ".png")
        
    else:
      plot_reward_figure(rs_tds_mean, rs_tds_std, title="Reward During Reversal for Temporal Difference Learner",sname="figures/room_task_td_reward.png")
      plot_reward_figure(rs_rbs_mean, rs_rbs_std, title="Reward During Reversal for Reward Basis Learner",sname="figures/room_task_rb_reward.png")
      if use_successor_agent:
...
def plot_successor_matrix_evolution(learning_rate, beta, gamma, N_iterations, plot_N):
    ...
    SR_agent = SuccessorRepresentationLearner(gamma, room_r2, env, learning_rate, beta, random_policy = True)
    ...
    columns = 5
    rows = 4
    fig, ax_array = plt.subplots(rows, columns,squeeze=False,figsize=(12,10))
    ...
    plt.tight_layout()
    plt.savefig("figures/successor_evolution_2.png")
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("figures/VF_evolution_2.png")
    plt.show()
...
def convergence_times(learning_rate, beta, gamma, N_iterations, N_runs):
    ...
    for n in range(N_runs):
      ...
      print("VSS: ", Vss.shape)
      print("MSS: ", Ms.shape)
      Vss = np.array([Vss[i,:,:] / np.sum(np.abs(Vss[i,:,:])) for i in range(len(Vss))])
      Ms = np.array([Ms[i,:,:] / np.sum(np.abs(Ms[i,:,:])) for i in range(len(Ms))])
      final_Vss = Vss[-1,:,:]
      final_Ms = Ms[-1,:,:]
      
      Vss_diffs = np.array([np.abs(Vss[i,:,:] - final_Vss) for i in range(len(Vss))])
      Vss_mean_diffs = np.sum(Vss_diffs, axis=(1,2))
      #Vss_std_diffs = np.std(Vss_diffs, axis=[1,2])
      
      Ms_diffs = np.array([np.abs(Ms[i,:,:] - final_Ms) for i in range(len(Ms))])
    ...
    plt.fill_between(xs, Vss_mean_diffs - Vss_std_diffs,Vss_mean_diffs + Vss_std_diffs, alpha=0.5)
    plt.plot(xs,Ms_mean_diffs, label="SR")
    plt.fill_between(xs, Ms_mean_diffs - Ms_std_diffs, Ms_mean_diffs + Ms_std_diffs, alpha=0.5)
    plt.legend(fontsize=22)
    plt.xlabel("Episode",fontsize=25)
    plt.ylabel("Mean absolute difference",fontsize=25)
    plt.title("Convergence of SR and RB",fontsize=30)
    sns.despine(left=False,top=True, right=True, bottom=False)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)
    plt.savefig("figures/SR_RB_convergence.png")
    plt.show()
...
def learning_rate_sweep_combined_graph(lrs, beta, gamma, steps_per_reversal, N_runs = 10, run_afresh = False,line_plot = False,fignum=0,intermediate_plots=False, use_homeostatic = True):
  ...
  if line_plot:
    ...
    fig = plt.figure(figsize=(12,10))
    sns.set_theme(context='talk',font='sans-serif',font_scale=1.0)
    plt.errorbar(xs,rb_rs, yerr = rb_std / np.sqrt(N_runs), label="RB", capsize=4)
    #plt.fill_between(xs, rb_rs - rb_std,rb_rs + rb_std, alpha=0.5)
    plt.errorbar(xs,sr_rs, yerr = sr_std / np.sqrt(N_runs), label="SR",capsize=4)
    ...
    plt.title("Reward Obtained against Learning Rate",fontsize=30)
    sns.despine(left=False,top=True, right=True, bottom=False)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)
    ...
    width = 0.2
    bar_rb = ax.bar(xs - (width + 0.02) , rb_rs, width, label="RB",alpha=0.9,yerr=rb_std)
    ...
    ax.set_xlabel("Learning Rate", fontsize=28)
    ax.set_title("Mean Reward obtained against learning rate",fontsize=30)
    ax.set_xticks(xs)
...
def steps_per_reversal_sweep_combined_graph(lr, beta, gamma, steps_per_reversal, N_runs = 10, run_afresh = False,line_plot = False,fignum=0,intermediate_plots=False, use_homeostatic = True,plot_intermediate_results = False):
  ...
  if line_plot:
    ...
    plt.xticks(ticks = xs,labels=lrs_labels, fontsize=22)
    plt.ylabel("Total Reward",fontsize=25)
    plt.title("Reward Obtained against Steps Per Reversal",fontsize=30)
    sns.despine(left=False,top=True, right=True, bottom=False)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)
    ...
    bar_sr = ax.bar(xs, sr_rs, width, label="SR",alpha=0.9,yerr=sr_std)
    ax.set_ylabel("Total Reward", fontsize=28)
    ax.set_xlabel("Steps per Reversal", fontsize=28)
    ax.set_title("Mean Reward obtained against steps per reversal",fontsize=30)
    ax.set_xticks(xs)
    ax.set_xticklabels(labels)
    ax.tick_params(axis='x', which='major', labelsize=25)
    ax.tick_params(axis='x', which='minor', labelsize=25)
    ...
    sns.despine(left=False,top=True, right=True, bottom=False)
    fig.tight_layout()
    plt.savefig("figures/interval_steps_maze_combined_bar_3.jpg", format="jpg")
    plt.show()
    
def plot_value_function(V,sname,title):
  fig = plt.figure()
  plt.imshow(V.reshape(6,6),vmin=-1, vmax=5)
  plt.xticks([])
  plt.yticks([])
...
def convergence_rb_td(beta, gamma, learning_rate,N_iterations, N_runs, coeff=0.33):
  ...
  for n in range(N_runs):
    ...
    RB_agent = Reward_Basis_Learner(gamma,[room_r1, room_r2, room_r3],env,learning_rate,beta,[1,0,0], random_policy=True)
    rb_rs, rb_Vs,rb_Vss = RB_agent.interact(N_iterations, return_rb_vlist=True)
  ...
  plt.legend(fontsize=22)
  plt.xlabel("Episode",fontsize=25)
  plt.ylabel("Total Squared Differences",fontsize=25)
  plt.title("Convergence of TD and RB estimated Value Functions",fontsize=30)
  sns.despine(left=False,top=True, right=True, bottom=False)
  plt.xticks(fontsize=22)
  plt.yticks(fontsize=22)
  plt.savefig("figures/combined_TD_RB_convergence.png")
  plt.show()
...
def room_sizes_sweep_graph(lr, beta, gamma, room_sizes, steps_per_reversal, N_runs = 10, run_afresh = False,line_plot = False,fignum=0,intermediate_plots=False):
  if run_afresh:
    ...
    for room_size in room_sizes:
      ...
      rs_tds, rs_rbs, rs_srs = plot_N_results(N_runs, lr, beta, gamma, steps_per_reversal, use_successor_agent=True,plot_results = True, preset_env = env)
      rb_rs.append(np.mean(rs_rbs,axis=1))
      td_rs.append(np.mean(rs_tds,axis=1))
      sr_rs.append(np.mean(rs_srs,axis=1))
      if intermediate_plots:
  ...
  if line_plot:
    ...
    #plt.fill_between(xs, td_rs - td_std, td_rs + td_std, alpha=0.5)
    plt.legend(fontsize=22)
    plt.xlabel("Room Size",fontsize=25)
    plt.xticks(ticks = xs,labels=lrs_labels, fontsize=22)
    plt.ylabel("Total Reward",fontsize=25)
    plt.title("Reward Obtained Against Room Size",fontsize=30)
    sns.despine(left=False,top=True, right=True, bottom=False)
    plt.xticks(fontsize=22)
    plt.yticks(fontsize=22)
    ...
    bar_sr = ax.bar(xs, sr_rs, width, label="SR",alpha=0.9,yerr=sr_std)
    ax.set_ylabel("Total Reward", fontsize=28)
    ax.set_xlabel("Room Size", fontsize=28)
    ax.set_title("Mean Reward obtained Against Room Size",fontsize=30)
    ax.set_xticks(xs)
    ax.set_xticklabels(labels)
    ax.tick_params(axis='x', which='major', labelsize=25)
    ax.tick_params(axis='x', which='minor', labelsize=25)
...
def verify_kappa_homeostatic():
  env = RoomEnv()
  homeostatic_agent = Homeostatic_TD_Learner(gamma,room_r1,env,learning_rate,beta)
  kappa = homeostatic_agent.set_kappa_reward_function()
  plt.imshow(kappa.reshape(6,6))
  plt.show()
